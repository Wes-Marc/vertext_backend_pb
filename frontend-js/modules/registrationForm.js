...
export default class RegistrationForm {
    ...
    events() {
        ...
        this.username.addEventListener("keyup", () => {
            this.valueHasChanged(this.username, this.usernameHandler);
        });

        this.email.addEventListener("keyup", () => {
            this.valueHasChanged(this.email, this.emailHandler);
        });

        this.password.addEventListener("keyup", () => {
            this.valueHasChanged(this.password, this.passwordHandler);
        });
    ...
    valueHasChanged(element, handler) {
    ...
    async usernameAfterDelay() {
        ...
        if (!this.username.errors) {
            try {
                const response = await axios.post("/doesUsernameExist", { username: this.username.value });
                if (response.data) {
